...
module.exports = {
    ...
    auth: function(req, res) {
        ...
        Usuario.find(userQuery).then(function(user) {
            if(user.length === 1) {
                user[0].validPassword(req.body.password).then(function(isValidPassword) {
                    if(isValidPassword)
                        res.redirect('/bicicletas/create');
                    else
                    res.status(401).render('usuarios/login', {errors: {credentials: 'Invalid credentials'}})
                });
            } else
            res.status(401).render('usuarios/login', {errors: {credentials: 'Invalid credentials'}})
        })
